# container-github-action-export-branches

A container that is used for [GitHub actions export-branches](https://github.com/ucomesdag/export-branches-action).

[![github-export-branches container image][quai.io]](https://quay.io/repository/ucomesdag/github-action-export-branches?tab=tags)

This container contains:
 - python3-request

<!-- container image -->

[quai.io]: https://img.shields.io/badge/Quay.io-container%20image-green?style=flat&logo=data:image/png;base64,&logoColor=white
